    ...
    try:
        ...
        if current_user.is_authenticated:
            notification_count = Notification.query.filter_by(
                recipient_user_id=current_user.id, is_read=False).count()
        else:
            session_id = session.get('session_id')
            if session_id:
                notification_count = Notification.query.filter_by(
                    recipient_session_id=session_id, is_read=False).count()
            else:
        ...
        if book:
            ...
            if book.posted_by:
                ...
                if poster:
                    ...
                    logging.info(
                        f"Added poster info for book {book_id}: {book_dict['poster']}"
                    )
                else:
                    logging.warning(
                        f"Poster user {book.posted_by} not found for book {book_id}"
                    )
            else:
...
def add_borrowed_book(book_id, proposed_due_date=None):
    ...
    try:
        ...
        existing = BorrowedBook.query.filter_by(book_id=book_id,
                                                user_id=current_user.id,
        ...
        if not existing:
            ...
            if proposed_due_date:
                try:
                    agreed_due_date = datetime.fromisoformat(
                        proposed_due_date.replace('Z', '+00:00'))
                except:
                    agreed_due_date = None
...
def create_borrow_request(book_id, proposed_due_date=None):
    ...
    try:
        ...
        create_borrow_notification(book, current_user.get_full_name(),
                                   due_date)

        db.session.commit()

        logging.info(
            f"Created borrow request for book {book_id} by user {current_user.id}"
        )
        return True
...
def create_borrow_notification(book, borrower_name, proposed_due_date=None):
    ...
    try:
        ...
        if book.posted_by:
            ...
            logging.info(
                f"Created borrow notification for book owner {book.posted_by} - book {book.title} requested by {borrower_name}"
            )
        else:
            logging.info(
                f"No notification created for book {book.title} - no owner tracked"
            )
    except Exception as e:
...
def index():
    ...
    if search_query:
        filtered_books = [
            ...
            or search_query in book.get('description', '').lower()
        ]
...
def borrow_book(book_id):
    ...
    if not current_user.is_authenticated:
        return jsonify({
            ...
            False,
            'message':
            'Please log in to borrow books. Real book borrowing requires an account.'
        }), 401
    ...
    existing_request = BorrowedBook.query.filter_by(book_id=book_id,
                                                    user_id=current_user.id,
                                                    is_returned=False).first()
    ...
    if success:
        return jsonify({
            ...
            True,
            'message':
            f'Borrow request sent for "{book["title"]}". Waiting for owner approval.'
        })
    else:
        return jsonify({
            'success': False,
...
def dashboard():
    ...
    try:
        ...
        approved_records = BorrowedBook.query.filter_by(
            user_id=current_user.id, is_returned=False, is_agreed=True).all()
    ...
    if request.method == 'POST':
        try:
            ...
            if not all([title, author, category]):
                flash('Title, Author, and Category are required fields.',
                      'danger')
                return render_template('post_book.html')

            # Validate category selection
            valid_categories = [
                'Tiểu thuyết', 'Phi tiểu thuyết', 'Trinh thám', 'Lãng mạn',
                'Khoa học viễn tưởng', 'Kỳ ảo', 'Tiểu sử', 'Lịch sử', 'Tự lực',
                'Kinh doanh', 'Công nghệ', 'Văn học cổ điển', 'Giáo dục',
                'Sức khỏe & Thể hình', 'Du lịch', 'Nấu ăn',
                'Nghệ thuật & Thiết kế', 'Triết học', 'Tôn giáo & Tâm linh',
                'Chính trị', 'Tâm lý học', 'Tội phạm có thật', 'Kinh dị',
                'Giật gân', 'Thanh thiếu niên', 'Thiếu nhi', 'Thơ', 'Kịch',
                'Truyện tranh & Tiểu thuyết đồ họa', 'Khác'
            ]
            ...
            try:
                ...
                flash('Publication year and pages must be valid numbers.',
                      'danger')
                return render_template('post_book.html')
            ...
            if extra_info:
                book.description = f"{book.description}\n\n{' | '.join(extra_info)}" if book.description else ' | '.join(
                    extra_info)
            ...
            flash(
                f'Book "{title}" has been successfully added to the catalog!',
                'success')
            return redirect(url_for('index'))
...
def seed_books():
    ...
    try:
        ...
        sample_books = [{
            ...
            'description':
            'A dystopian social science fiction novel about totalitarian control and the struggle for truth and freedom.',
            'cover_url':
        ...
        return jsonify({
            ...
            True,
            'message':
            f'Added {len(sample_books)} sample books to the catalog'
        })
    except Exception as e:
    ...
    try:
        if current_user.is_authenticated:
            ...
            user_notifications = Notification.query.filter_by(
                recipient_user_id=current_user.id).order_by(
            ...
            if session_id:
                user_notifications = Notification.query.filter_by(
                    recipient_session_id=session_id).order_by(
                        Notification.created_at.desc()).all()
            else:
                user_notifications = []
...
def mark_all_notifications_read():
    ...
    try:
        if current_user.is_authenticated:
            ...
            session_id = session.get('session_id')
            if session_id:
                Notification.query.filter_by(recipient_session_id=session_id,
                                             is_read=False).update(
                                                 {Notification.is_read: True})
...
def approve_borrow_request(book_id):
    ...
    try:
        ...
        borrow_record = BorrowedBook.query.filter_by(book_id=book_id,
                                                     user_id=borrower_id,
                                                     is_returned=False,
                                                     is_agreed=False).first()
        ...
        if agreed_due_date:
            borrow_record.agreed_due_date = datetime.fromisoformat(
                agreed_due_date.replace('Z', '+00:00'))
        ...
        if borrower:
            ...
            notification.borrower_name = current_user.get_full_name(
            ) or current_user.username
...
def cancel_borrow_request(book_id):
    ...
    try:
        ...
        logging.info(
            f"Cancelled borrow request for book {book_id} by user {current_user.id}"
        )
        ...
        return jsonify({
            ...
            True,
            'message':
            f'Cancelled borrow request for "{book_title}"'
        })
...
def book_discussion(book_id):
    ...
    if request.method == 'POST':
        try:
            ...
            if current_user.is_authenticated:
                ...
                if not username:
                    flash('Please enter your name.', 'warning')
                    return redirect(url_for('book_discussion',
                                            book_id=book_id))
                user_id = None
                session_id = get_user_identifier()
    ...
    try:
        messages = Discussion.query.filter_by(book_id=book_id).order_by(
            Discussion.created_at.asc()).all()
        return render_template('book_discussion.html',
                               messages=messages,
                               book=book)
    except Exception as e:
...
def private_chat(recipient_id, book_id=None):
    ...
    try:
        ...
        messages = PrivateMessage.query.filter((
            ...
            & (PrivateMessage.recipient_id == recipient_id)) | (
                (PrivateMessage.sender_id == recipient_id)
                & (PrivateMessage.recipient_id == current_user.id))).order_by(
                    PrivateMessage.created_at.asc()).all()

        # Mark messages as read
        PrivateMessage.query.filter_by(sender_id=recipient_id,
...
def add_review(book_id):
    ...
    if request.method == 'POST':
        try:
            ...
            if existing_review:
                ...
                # Create new review
                review = BookReview(book_id=book_id,
                                    user_id=current_user.id,
                                    rating=rating,
    ...
    user_review = BookReview.query.filter_by(book_id=book_id,
...
def get_reviews(book_id):
    ...
    try:
        reviews = BookReview.query.filter_by(book_id=book_id).order_by(
            BookReview.created_at.desc()).all()
        return jsonify({
            'success': True,
...
def check_and_award_achievements(user_id):
    ...
    try:
        ...
        unlocked_achievement_ids = db.session.query(
            UserAchievement.achievement_id).filter_by(user_id=user_id).all()
        unlocked_ids = [ua.achievement_id for ua in unlocked_achievement_ids]

        available_achievements = Achievement.query.filter(
            Achievement.is_active == True,
            ~Achievement.id.in_(unlocked_ids)).all()
        ...
        if newly_unlocked:
            db.session.commit()
            logging.info(
                f"Awarded {len(newly_unlocked)} new achievements to user {user_id}"
            )
    ...
    try:
        ...
        if achievement.requirement_type == 'count':
            if achievement.category == 'books':
                ...
                count = BorrowedBook.query.filter_by(user_id=user_id,
...
def get_user_achievements(user_id):
    ...
    try:
        ...
        user_achievements = db.session.query(
            UserAchievement, Achievement).join(
                Achievement,
                UserAchievement.achievement_id == Achievement.id).filter(
                    UserAchievement.user_id == user_id).order_by(
                        UserAchievement.unlocked_at.desc()).all()
        ...
        for user_ach, achievement in user_achievements:
            ...
            ) if user_ach.unlocked_at else None
...
def get_achievement_progress(user_id):
    ...
    try:
        ...
        all_achievements = Achievement.query.filter(
            Achievement.is_active == True).all()
                ...
                if achievement.requirement_type == 'count':
                    if achievement.category == 'books':
                        current_progress = BorrowedBook.query.filter_by(
                            user_id=user_id, is_returned=True).count()
                    elif achievement.category == 'reviews':
                        current_progress = BookReview.query.filter_by(
                            user_id=user_id).count()
                    elif achievement.category == 'social':
                        current_progress = Discussion.query.filter_by(
                            user_id=user_id).count()
                elif achievement.requirement_type == 'special':
                    current_progress = 1 if check_achievement_requirement(
                        user_id, achievement) else 0
            else:
...
def achievements():
    ...
    UserAchievement.query.filter_by(user_id=current_user.id,
                                    is_seen=False).update({'is_seen': True})
    db.session.commit()
...
def ranks():
    ...
    leaderboard = db.session.query(
        ...
        db.func.sum(Achievement.points).label('total_points')).join(
            UserAchievement, User.id == UserAchievement.user_id).join(
                Achievement,
                UserAchievement.achievement_id == Achievement.id).group_by(
                    User.id).order_by(db.desc('total_points')).limit(10).all()
    ...
    try:
        ...
        if cost > user_points:
            return jsonify({
                ...
                False,
                'message':
                f'Not enough points! You need {cost} points but only have {user_points}.'
            }), 400
        ...
        logging.info(
            f"Profile updated for user {current_user.id}, cost: {cost} points")
...
def seed_achievements():
    ...
    try:
        ...
        sample_achievements = [{
            'name': 'First Steps',
            'description':
            'Welcome to ReadingTrail! You\'ve joined our reading community.',
            'icon': 'fa-star',
            'category': 'special',
            ...
            'description':
            'Send your first private message to another reader.',
        ...
        return jsonify({
            ...
            True,
            'message':
            f'Successfully seeded {len(sample_achievements)} achievements'
        })
        ...
        return jsonify({
            'success': False,
            'message': f'Error seeding achievements: {str(e)}'
        }), 500
